...
### Canale Email: Resend (`lib/resendService.ts`)
- Invio istantaneo di email transazionali responsive di lusso (palette ufficiale Royal Violet `#5E1788`, Pastel Lilac `#D8C2E7`, Optical White `#FFFFFF`, Mauve Rose `#D462A6`):
  - Conferma prenotazione con riepilogo acconto 20% e saldo 80%, link Google Calendar e Apple Calendar (.ics);
  - Link per disdetta o modifica entro 24 ore;
  - Promemoria automatico a 24h dall'appuntamento con vademecum pelle pulita;
  - Notifica conferma ordine cosmetico con dettagli spedizione / ritiro boutique.
  - Modalità preview/simulazione realistica locale senza bloccare l'interfaccia se manca `RESEND_API_KEY`.

### Canale WhatsApp: Evolution API (`lib/whatsappQueueService.ts`)
- Collegamento del numero WhatsApp ufficiale del negozio (`+39 348 381 6516`) con sessione QR code SVG.
- Monitor di controllo real-time in `/admin/appuntamenti` nel tab dedicato *"📡 Canali Notifiche & Coda"* (`components/admin/NotificationQueueTab.tsx`).
